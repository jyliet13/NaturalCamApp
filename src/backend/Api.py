...
# ✅ CRÍTICO: Desactivar GPU para evitar problemas de memoria virtual en Windows
# Esto DEBE estar ANTES de importar tensorflow
os.environ['CUDA_VISIBLE_DEVICES'] = '-1'  # Fuerza uso de CPU solo
...
def predict():
    ...
    if file and allowed_file(file.filename):
        try:
            ...
            print(f"Imagen cargada: {file.filename}")
            ...
            return jsonify({
                ...
                "all_probabilities": {str(name): round(float(prob)*100, 2) for name, prob in zip(class_names, probabilities)}
            }), 200
